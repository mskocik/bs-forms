...
namespace Mskocik\Forms\Rendering;
...
class BootstrapRenderer extends DefaultFormRenderer
{
	...
	private function controlsInit()
	{
		...
		if (in_array($this->layout, [FormLayout::INLINE, FormLayout::INLINE_NO_LABELS])) {
		...
		foreach ($this->form->getControls() as $control) {
            ...
            if ($this->layout === FormLayout::VERTICAL &&
                ($control instanceof Controls\TextBase || $control instanceof Controls\SelectBox || $control instanceof Controls\MultiSelectBox)
            ) {
                $control->getLabelPrototype()->addClass('form-label');
            }
            if ($this->layout === FormLayout::INLINE_NO_LABELS &&
                ($control instanceof Controls\TextBase || $control instanceof Controls\SelectBox || $control instanceof Controls\MultiSelectBox)
            ) {
                $control->getLabelPrototype()->addClass('visually-hidden');
            }
